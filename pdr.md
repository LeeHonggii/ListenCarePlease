...
- **[6. 화자 태깅]**
    - **현황:** 설계 완료 (I,O.md 참조)
    - **로직 (5a~5f 단계):**
        1. **[5a~5c 내부 처리]** 이름 감지 → 문맥/화자 매칭 → 음성 임베딩 유사도 비교 (동일인 판별)
        2. **[5d UI 제안]** 백엔드가 판단한 결과를 단순화하여 사용자에게 제시:
            - `detected_names`: 감지된 모든 이름 리스트
            - `suggested_mappings`: 각 SPEAKER_XX에 대한 추천 이름
...
**[진행 예정] Step 6: React UI 구현**
- React UI 구현 (4. UX/UI 설계 기반)
    - 로그인/회원가입 페이지
    - 파일 업로드 페이지 (드래그앤드롭)
    - 화자 태깅 UI (5d~5e 플로우)
    - 결과 대시보드
    - 내 파일 목록 페이지
- 세련된 디자인 적용 (TailwindCSS, Glassmorphism, 애니메이션)
