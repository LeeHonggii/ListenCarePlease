...
4. **Typo Fix Only:** 오직 철자 오류(예: 쓰레솔드 -> 쓰레숄드)만 교정하십시오.
...
def extract_keywords(state: AgentState) -> AgentState:
    """Calls the LLM to extract keywords."""
    text = state["text"]
    
    # Limit text length to avoid token limits (approx 60k chars as per notebook)
